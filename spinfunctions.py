...
class SpinFunctions:
    ...
    def get_sigma2(self):
        ...
        def ALEX(sigma2_disc, sigma2_high, Ex = Ex):
            '''
            Pure linear function of spin-cutoff parameter between a low and a 
            high value.
            ...
            '''
            x = [sigma2_disc[0], sigma2_high[0]]
            y = [sigma2_disc[1], sigma2_high[1]]
            sigma2 = interp1d(x, y,
                              bounds_error=False,
                              fill_value=(sigma2_disc[1], sigma2_high[1]))
            return sigma2(Ex)
        ...
        if model == "ALEX":
            pars_req = {"sigma2_disc", "sigma2_high"}
            return call_model(ALEX, pars, pars_req)
        else:
            raise TypeError(
